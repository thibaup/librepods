...
# 0.0.x are the current exporter fixtures. 1.0.0 is the legacy public-schema example and uses
# the original paired-plist layout; it is accepted only when no 0.0.3 custom entries are present.
_SUPPORTED_OPENTAGVIEWER_EXPORT_VERSIONS = {"0.0.1", "0.0.2", "0.0.3", "1.0.0"}


def _safeImportedLabel(value: Any, fallback: str) -> str:
...
def validateOpenTagViewerManifest(manifestYaml: str, hasCustomAccessories: bool) -> str:
    """Validate the small metadata document before any imported owner key is persisted."""
    try:
        document = yaml.safe_load(manifestYaml)
        if not isinstance(document, dict):
            raise ValueError("The OpenTagViewer manifest must be a mapping.")

        required = {"version", "via", "sourceUser", "exportTimestamp"}
        if not required.issubset(document):
            raise ValueError("The OpenTagViewer manifest is missing required fields.")

        version = document["version"]
        if not isinstance(version, str) or not re.fullmatch(r"(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)", version):
            raise ValueError("The OpenTagViewer export version is invalid.")
        if version not in _SUPPORTED_OPENTAGVIEWER_EXPORT_VERSIONS:
            raise ValueError(f"OpenTagViewer export version {version} is not supported yet.")
        if not isinstance(document["via"], str) or not document["via"].strip():
            raise ValueError("The OpenTagViewer export producer is missing.")
        if not isinstance(document["sourceUser"], str) or not document["sourceUser"].strip():
            raise ValueError("The OpenTagViewer export owner is missing.")
        timestamp = document["exportTimestamp"]
        if isinstance(timestamp, bool) or not isinstance(timestamp, int) or timestamp < 0:
            raise ValueError("The OpenTagViewer export timestamp is invalid.")
        if hasCustomAccessories and version != "0.0.3":
            raise ValueError("This export contains custom accessories but does not declare format 0.0.3.")

        return json.dumps({"ok": True, "version": version})
    except ValueError as error:
        print("OpenTagViewer manifest validation failed")
        return json.dumps({"ok": False, "message": str(error)})
    except Exception:
        print("OpenTagViewer manifest validation failed")
        return json.dumps({"ok": False, "message": "The OpenTagViewer manifest is invalid."})


def describeImportedPlists(ownedBeaconPlist: str, namingRecordPlist: str) -> str:
    """Return a display label without exposing any private plist fields to Java logs."""
    try:
        owned = util_files.read_data_plist(ownedBeaconPlist.encode("utf-8"))
        naming = util_files.read_data_plist(namingRecordPlist.encode("utf-8"))
        label = naming.get("name") if isinstance(naming, dict) else None
        if not isinstance(label, str) or not label.strip():
            label = owned.get("model") if isinstance(owned, dict) else None
        return json.dumps({"ok": True, "label": _safeImportedLabel(label, "Imported accessory")})
    except Exception:
        print("Imported accessory label could not be read")
        return json.dumps({"ok": True, "label": "Imported accessory"})


def _importedRecordTimestamp(value: Any) -> float | None:
...
def _readImportedPlistRecords(recordsJson: str, beaconIdentifier: str, kind: str) -> list[dict]:
    raw_records = json.loads(recordsJson)
    if not isinstance(raw_records, list):
        raise ValueError("Imported plist records must be a list.")

    records = []
    for raw in raw_records:
        if not isinstance(raw, dict) or set(raw) != {"recordId", "plist"}:
            raise ValueError("An imported plist record has an invalid shape.")
        record_id = raw["recordId"]
        plist_text = raw["plist"]
        if not isinstance(record_id, str) or not isinstance(plist_text, str):
            raise ValueError("An imported plist record has invalid fields.")
        record = util_files.read_data_plist(plist_text.encode("utf-8"))
        if not isinstance(record, dict):
            raise ValueError("An imported plist record is not a mapping.")
        if str(record.get("identifier", "")).strip().upper() != record_id:
            raise ValueError("An imported plist record identifier does not match its filename.")

        association_field = "associatedBeacon" if kind == "naming" else "beaconIdentifier"
        associated = record.get(association_field)
        if isinstance(associated, str) and associated.strip().upper() != beaconIdentifier:
            raise ValueError("An imported plist record belongs to a different accessory.")

        records.append({"recordId": record_id, "plist": plist_text, "mapping": record})
    return records


def _latestImportedNamingRecord(records: list[dict]) -> dict:
    if not records:
        raise ValueError("An imported accessory has no naming record.")

    def sort_key(index_and_record):
        index, record = index_and_record
        modified = None
        metadata = record["mapping"].get("cloudKitMetadata")
        if isinstance(metadata, (bytes, bytearray)):
            try:
                decoded = NSKeyedUnArchiver.unserializeNSKeyedArchiver(bytes(metadata))
                if isinstance(decoded, dict):
                    modified = _importedRecordTimestamp(
                        decoded.get("RecordMtime") or decoded.get("RecordCtime")
                    )
            except Exception:
                # Old exports can carry metadata in an unknown keyed-archive revision. The
                # deterministic record-id order remains a safe display-name fallback.
                modified = None
        return (modified is not None, modified or 0.0, index)

    return max(enumerate(records), key=sort_key)[1]


def _latestImportedAlignmentRecord(records: list[dict]) -> dict | None:
    if not records:
        return None
    for record in records:
        mapping = record["mapping"]
        observed = _importedRecordTimestamp(mapping.get("lastIndexObservationDate"))
        index = mapping.get("lastIndexObserved")
        if (
            observed is None
            or isinstance(index, bool)
            or not isinstance(index, int)
            or not 0 <= index <= 500_000
        ):
            raise ValueError("An imported key-alignment record is invalid.")
        record["observedTimestamp"] = observed
    return max(records, key=lambda record: (record["observedTimestamp"], record["recordId"]))


def prepareImportedPairedAccessory(
    ownedBeaconPlist: str,
    beaconIdentifier: str,
    namingRecordsJson: str,
    alignmentRecordsJson: str,
) -> str:
    """Validate, select and convert all records belonging to one paired accessory."""
    try:
        owned = util_files.read_data_plist(ownedBeaconPlist.encode("utf-8"))
        if not isinstance(owned, dict):
            raise ValueError("An OwnedBeacons record is not a mapping.")
        if str(owned.get("identifier", "")).strip().upper() != beaconIdentifier:
            raise ValueError("An OwnedBeacons identifier does not match its filename.")

        naming_records = _readImportedPlistRecords(
            namingRecordsJson, beaconIdentifier, "naming"
        )
        alignment_records = _readImportedPlistRecords(
            alignmentRecordsJson, beaconIdentifier, "alignment"
        )
        naming = _latestImportedNamingRecord(naming_records)
        alignment = _latestImportedAlignmentRecord(alignment_records)

        if alignment:
            paired_at = _importedRecordTimestamp(owned.get("pairingDate"))
            observed_at = alignment["observedTimestamp"]
            alignment_index = alignment["mapping"]["lastIndexObserved"]
            if paired_at is None or observed_at < paired_at:
                raise ValueError("An imported alignment predates its accessory pairing.")
            # A Find My accessory advances no faster than once per 15 minutes. This also prevents
            # a hostile index from making FindMy.py perform an unbounded KDF loop during locate.
            plausible_max_index = int((observed_at - paired_at) // (15 * 60)) + 96
            if alignment_index > plausible_max_index:
                raise ValueError("An imported key-alignment index is not plausible.")

        alignment_bytes = alignment["plist"].encode("utf-8") if alignment else None
        accessory = FindMyAccessory.from_plist(BytesIO(ownedBeaconPlist.encode("utf-8")), alignment_bytes)
        accessory.keys_at(0)
        label = naming["mapping"].get("name")
        if not isinstance(label, str) or not label.strip():
            label = owned.get("model")
        return json.dumps(
            {
                "ok": True,
                "label": _safeImportedLabel(label, "Imported accessory"),
                "accessoryJson": json.dumps(accessory.to_json()),
            }
        )
    except Exception:
        print("Imported paired accessory validation failed")
        return json.dumps(
            {"ok": False, "message": "A paired accessory in the export is invalid."}
        )


_CUSTOM_ACCESSORY_MAX_KEYS = 255
_CUSTOM_ACCESSORY_FILENAME_UNSAFE = re.compile(r"[^A-Za-z0-9._-]")


def validateCustomAccessoryJson(accessoryJson: str, expectedIdentifier: str) -> str:
    """Validate and canonicalise FindMy.py's self-generated rolling-key format."""
    try:
        mapping = json.loads(accessoryJson)
        if not isinstance(mapping, dict) or mapping.get("type") != "custom_rolling_key_accessory":
            raise ValueError("A CustomAccessories entry has an unsupported accessory type.")

        name = mapping.get("name")
        if not isinstance(name, str) or not name.strip() or len(name) > 160:
            raise ValueError("A custom accessory needs a short, non-empty name.")

        identifier = mapping.get("identifier")
        if not isinstance(identifier, str) or not identifier.strip() or len(identifier) > 512:
            raise ValueError("A custom accessory identifier is invalid.")
        safe_identifier = _CUSTOM_ACCESSORY_FILENAME_UNSAFE.sub("_", identifier.strip())
        if (
            not safe_identifier.strip("._")
            or len(safe_identifier) > 100
            or safe_identifier != expectedIdentifier
        ):
            raise ValueError("A custom accessory identifier does not match its filename.")

        private_keys = mapping.get("private_keys")
        if (
            not isinstance(private_keys, list)
            or not 1 <= len(private_keys) <= _CUSTOM_ACCESSORY_MAX_KEYS
        ):
            raise ValueError("A custom accessory has an invalid number of private keys.")

        decoded_keys = []
        for private_key in private_keys:
            if not isinstance(private_key, str) or not re.fullmatch(r"[0-9A-Fa-f]{56}", private_key):
                raise ValueError("A custom accessory contains an invalid private key.")
            decoded_keys.append(bytes.fromhex(private_key))
        if len(set(decoded_keys)) != len(decoded_keys):
            raise ValueError("A custom accessory contains duplicate private keys.")

        accessory = accessoryFromJson(accessoryJson)
        # FindMy.py's deserializer accepts arbitrary byte strings. Constructing the key pairs is
        # the actual curve/scalar validation and is deliberately done once, before persistence.
        accessory.keys_at(0)
        canonical = accessory.to_json()
        label = _safeImportedLabel(name, "Imported custom tag")
        return json.dumps({"ok": True, "label": label, "accessoryJson": json.dumps(canonical)})
    except Exception:
        print("Custom accessory validation failed")
        return json.dumps({"ok": False, "message": "A custom accessory in the export is invalid."})
